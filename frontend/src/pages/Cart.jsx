...
export default function Cart() {
  const { cartItems, removeFromCart, addToCart, isLoggedIn } =
  ...
  return (
    <main className="flex-body">
      ...
      <Container className="my-5">
        {cartItems.map((item, index) => {
          return (
            <Container
              fluid
              className="border-bottom border-light-subtle bg-body-tertiary"
              key={index}
            >
              <Row className="p-2">
                <Col
                  ...
                  <Row fluid className="h-100 d-flex align-items-center">
                    ...
                    <Col fluid lg={3} md={3} sm={8} xs={9} className="d-flex">
                      <Container fluid className="d-flex cart-btns-container">
                        ...
                        <Form.Control
                          className="text-center  input-control"
                          type="text"
                          value={item.quantity}
                          disabled
                        />
                    ...
                    <Col fluid lg={1} md={1} sm={4} xs={3}>
                      <Button
                        ...
                        to="#"
                      >
                        <Image src="/assets/cart_cross_icon.png" />
                      </Button>
                    </Col>
                  </Row>
                </Col>
      ...
      <Container className="text-center bg-secondary-subtle">
        <p>
          <span className="fs-2">Total amount: </span>
          <span className="fs-1 text-primary">₹{total}</span>
        </p>
      </Container>
      <Container className="text-center mb-5">
        <Button
          onClick={() => (total === 0 ? null : alert("Purchase successful"))}
          variant="primary"
          className="btn-theme fs-4"
        >
          Purchase
        </Button>
      </Container>
